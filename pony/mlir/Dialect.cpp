//===- Dialect.cpp - Pony IR Dialect registration in MLIR ------------------===//
//
//===----------------------------------------------------------------------===//
//
// This file implements the dialect for the Pony IR: custom type parsing and
...
using namespace mlir::pony;

#include "pony/Dialect.cpp.inc"

//===----------------------------------------------------------------------===//
// PonyInlinerInterface
//===----------------------------------------------------------------------===//

/// This class defines the interface for handling inlining with Pony
/// operations.
struct PonyInlinerInterface : public DialectInlinerInterface {
  ...
  /// All call operations within pony can be inlined.
  bool isLegalToInline(Operation *call, Operation *callable,
                       bool wouldBeCloned) const final {
    return true;
  }

  /// All operations within pony can be inlined.
  bool isLegalToInline(Operation *, Region *, bool,
                       BlockAndValueMapping &) const final {
    return true;
  }

  // All functions within pony can be inlined.
  ...
  /// Handle the given inlined terminator(pony.return) by replacing it with a new
  /// operation as necessary.
  void handleTerminator(Operation *op,
                        ArrayRef<Value> valuesToRepl) const final {
    // Only "pony.return" needs to be handled here.
...
void PonyDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "pony/Ops.cpp.inc"
      >();
  addInterfaces<PonyInlinerInterface>();
}
